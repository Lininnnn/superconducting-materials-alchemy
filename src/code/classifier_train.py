...
DATA_DIR = PROJ_ROOT / 'data/graph_tensors_rbf' 
...
def run_pretrain_pipeline():
    ...
    raw_dataset = [torch.load(p, weights_only=False) for p in tqdm(pt_files, desc="data loading")]
    ...
    hybrid_cls = XGBHybridClassifier()
    ...
    print("\n" + "="*45)
    print(f"classification result (ACC): {acc:.2%}")
    ...
    print(f"\nstatistical magnitude: master_stats.pth\n- classifier: best_classifier.pkl\n- Regressor: best_regressor.pth")
